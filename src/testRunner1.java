	  ...
	  int errorLines = 0;
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                result += number;
                finalString.append(number >= 0 ? MINUS : PLUS).append(Math.abs(number));
                ...
                final char CHAR_MIN = '-';
                String sym = finalString.substring(0,SINGL);
                finalString.delete(0, SINGL);
                if (sym.equals(MINUS)) {
                	finalString.insert(0, CHAR_MIN);
                }
                }
            finalString.insert(0,RESULT_HEAD).append(RESULT_TAIL).append(result);
        }finally {
		}  
        return errorLines;
    }
